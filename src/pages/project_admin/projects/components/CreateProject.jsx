// ═══════════════════════════════════════════════════════════════════
// CREATE PROJECT — New project only. Never touches _id or update mutation.
// Props:
//   onCreate  – async (project, status) => void
//   onCancel  – () => void
// ═══════════════════════════════════════════════════════════════════
...
import MeasurementFieldEditor from "./sub-components/MeasurementFieldEditor.jsx";
import { PROJECT_QUERIES } from "../../../../apollo/gql.js";
import { deepClean } from "../../../../utils/helpers.js";

export function CreateProject({ onCreate, onCancel }) {
  const [proj, setProj] = useState(newProject);
  const [activeChapterId, setActCh] = useState(null);
  ...
  const handleCreate = async (status = "ACTIVE") => {
    ...
    setApiError("");
    try {
      const cleaned = deepClean({ ...proj, status });
      const result = await createProject({
        variables: { projectInput: cleaned },
      });
      onCreate({ ...proj, _id: result.data.createProject._id, status });
    } catch (err) {
...
export function ProjectDetailsCard({ proj, errors, apiError, onChange }) {
  return (
    <div className="card" style={{ marginBottom: 16 }}>
      ...
      <FormField label="Allotted Budget ₹">
        <input
          ...
          type="number"
          step="1"
          value={proj.Vault?.allotedBudjet ?? 0}
          onChange={(e) =>
            onChange("Vault.allotedBudjet", Number(e.target.value))
          }
        ...
        {!activeChapter ? (
          ...
          <>
                  ...
                  >
                    {CHAPTER_COLORS.map((c) => (
                      <div
                        key={c}
                        onClick={() =>
                          updateChapter(setProj, activeChapter._id, "color", c)
                        }
                        style={{
                          width: 20,
                          height: 20,
            ...
            >
              ...
              <button
                className="btn btn-primary btn-sm"
                onClick={() => addItem(setProj, setActItem, activeChapter._id)}
              >
                +
              </button>
            </div>
                ...
                >
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      >
                        <code style={{ fontSize: 10 }}>{item.code || "—"}</code>{" "}
                    ...
                    <div style={{ display: "flex", gap: 2 }}>
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          moveItem(setProj, activeChapter._id, item._id, -1);
                        }}
                      >
                        ▲
                      </button>
