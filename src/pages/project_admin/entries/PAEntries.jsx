...
import EntryActionModal from "./components/EntryActionModal.jsx";
...
const STATUS_OPTIONS = ["SUBMITTED", "APPROVED", "REJECTED", "RETURNED"];

const STATUS_COLORS = {
  SUBMITTED: { bg: "rgba(59,130,246,.12)",  text: "#60A5FA",       border: "rgba(59,130,246,.25)"  },
  APPROVED:  { bg: "rgba(34,197,94,.12)",   text: "var(--green)",  border: "rgba(34,197,94,.25)"   },
  REJECTED:  { bg: "rgba(239,68,68,.12)",   text: "var(--red)",    border: "rgba(239,68,68,.25)"   },
  RETURNED:  { bg: "rgba(234,179,8,.12)",   text: "var(--yellow)", border: "rgba(234,179,8,.25)"   },
};
...
const hasActiveFilters = (f) =>
  f.status || f.projects.length || f.spans.length ||
  f.createdBy.length || f.fromDate || f.toDate;

const activeCount = (f) =>
  (f.status ? 1 : 0) +
  f.projects.length + f.spans.length + f.createdBy.length +
...
export default function PAEntries() {
  ...
  // ── Facets ────────────────────────────────────────────────────────
  const { data: facetData, loading: facetLoading } = useQuery(EMB_ENTRY.facets, {
    variables: {
        ...
        actions={
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button
              ...
              title="Open progress report for the current filter selection"
            >
              📊 Progress Report
            </button>
          </div>
...
// FILTER PANEL
// ═══════════════════════════════════════════════════════════════════
function FilterPanel({ filters, facets, facetLoading, onChange, onReset, onClose }) {
  return (
    <div
      className="card"
      style={{ marginBottom: 14, padding: 18, border: "1px solid var(--accent)44" }}
    >
      ...
      <Section label="Project">
        <FacetMultiSelect
          ...
          onChange={(id) => onChange({ projects: toggleArr(filters.projects, id) })}
        />
      </Section>
      ...
      <Section label="Field engineer">
        <FacetMultiSelect
          ...
          getCount={(f)    => f.count}
          loading={facetLoading}
          onChange={(id) => onChange({ createdBy: toggleArr(filters.createdBy, id) })}
        />
      </Section>
    </div>
  );
...
function FacetMultiSelect({ items, selected, getId, getLabel, getSubLabel, getCount, loading, onChange }) {
        ...
        return (
          <div
            ...
            style={{
              ...
              background: active ? "rgba(244,160,28,.1)"  : "rgba(255,255,255,.02)",
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{
                ...
                {active && <span style={{ fontSize: 10, color: "var(--navy)", fontWeight: 700 }}>✓</span>}
              </div>
              <div>
                <div style={{ fontSize: 12, fontWeight: 600, color: active ? "var(--accent)" : "var(--text)" }}>
                  {getLabel(item)}
                </div>
                <div style={{ fontSize: 10, color: "var(--text3)" }}>{getSubLabel(item)}</div>
              </div>
            </div>
            <span style={{
              fontSize: 11, fontWeight: 700, minWidth: 24, textAlign: "center",
              padding: "1px 6px", borderRadius: 10,
              background: active ? "rgba(244,160,28,.2)" : "rgba(255,255,255,.06)",
...
function ActiveChips({ filters, facets, onChange, onReset }) {
  ...
  if (filters.fromDate || filters.toDate) chips.push({
    key: "date", label: `${filters.fromDate || "…"} → ${filters.toDate || "…"}`,
    onRemove: () => onChange({ fromDate: "", toDate: "" }),
  });
  filters.projects.forEach((id) => {
    const f = facets.projectCount.find((x) => x.project._id === id);
    chips.push({ key: `proj-${id}`, label: f?.project.name ?? id,
      onRemove: () => onChange({ projects: filters.projects.filter((x) => x !== id) }) });
  });
  filters.spans.forEach((id) => {
    const f = facets.spanCount.find((x) => x.span._id === id);
    chips.push({ key: `span-${id}`, label: f?.span.name ?? id,
      onRemove: () => onChange({ spans: filters.spans.filter((x) => x !== id) }) });
  });
  filters.createdBy.forEach((id) => {
    const f = facets.createdByCount.find((x) => x.createdBy._id === id);
    chips.push({ key: `by-${id}`, label: f?.createdBy.name ?? id,
      onRemove: () => onChange({ createdBy: filters.createdBy.filter((x) => x !== id) }) });
  });
    ...
    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12, alignItems: "center" }}>
      ...
      {chips.map((chip) => (
        <span key={chip.key} style={{
          ...
          background: "rgba(244,160,28,.12)", border: "1px solid rgba(244,160,28,.3)",
          color: "var(--accent)",
        }}>
          {chip.label}
          <button onClick={chip.onRemove} style={{
            background: "none", border: "none", cursor: "pointer",
            color: "var(--accent)", fontSize: 13, lineHeight: 1, padding: 0,
          }}>×</button>
        </span>
      ))}
...
function Pagination({ page, totalPages, onChange }) {
  ...
  return (
    <div style={{
      display: "flex", alignItems: "center", justifyContent: "center",
      gap: 4, padding: "14px 0 0", borderTop: "1px solid var(--border)",
    }}>
      <button className="btn btn-ghost btn-sm" disabled={page === 1}
        onClick={() => onChange(page - 1)} style={{ padding: "4px 10px" }}>
        ← Prev
      </button>
      {display.map((p, i) =>
        p === "…"
          ? <span key={`el-${i}`} style={{ fontSize: 12, color: "var(--text3)", padding: "0 4px" }}>…</span>
          : <button key={p} onClick={() => onChange(p)} style={{
              width: 32, height: 32, borderRadius: 6, fontSize: 12, fontWeight: 600,
              cursor: "pointer", border: "1px solid",
              borderColor: p === page ? "var(--accent)" : "var(--border)",
              background:  p === page ? "rgba(244,160,28,.15)" : "rgba(255,255,255,.03)",
...
function MiniSpinner() {
  return <div style={{ fontSize: 11, color: "var(--text3)", padding: "4px 0" }}>Loading options…</div>;
}

const labelStyle = {
  fontSize: 10, color: "var(--text2)", textTransform: "uppercase",
  letterSpacing: ".07em", marginBottom: 6, fontWeight: 600,
};