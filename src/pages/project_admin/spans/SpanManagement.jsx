...
export default function SpanManagement() {
  ...
  const saveSpan = async (span) => {
    if (span._id) {
      ...
      console.log("Span Input", spanInput);
      await createSpan({
        variables: { spanInput },
        update(cache) {
        ...
        {step === 1 && (
          <div>
            ...
            {project && (
              <AlertBanner
                type="info"
                message={`${project.chapters.map((c) => c.items).length} work items will be imported from "${project.name}"`}
              />
            )}
              ...
              <div className="form-group">
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {(projectsData?.projects?.data ?? [])
                    .find((t) => t._id === span.projectId)
                    ?.chapters.map((opt) => {
                      const active = (
                        span.chapters.map((t) => t._id) ?? []
                      ).includes(opt._id);
                      return (
                        <button
                          key={opt.name}
                          ...
                          onClick={(e) => {
                            set(
                              "chapters",
                              span.chapters?.find((t) => t._id === opt._id)
                                ? span.chapters.filter((t) => t._id !== opt._id)
                                : [...(span.chapters ?? []), opt],
                            );
                          }}
                          style={{
            ...
            {[
              ...
              >
                ...
                <FormField
                  label="Place Name"
                  error={
                    key === "startPoint" ? errors.startName : errors.endName
                  }
                  required
                >
                  <input
