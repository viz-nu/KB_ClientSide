...
export default function MyEntries() {
  ...
  return (
    <div className="fade-up">
      <PageHeader
        title="My Entries"
        subtitle={loading ? "Loading…" : `${pagination.totalDocuments ?? entries.length} entries`}
        actions={
        ...
        <div className="table-wrap" style={{ position: "relative", minHeight: 120 }}>
          ...
          {!loading && !error && entries.length === 0 && (
            <EmptyState
              ...
              message={active ? "Try adjusting your filters." : "You haven't submitted any entries yet."}
              action={
                active
                  ? <button className="btn btn-outline" onClick={resetFilters}>Clear Filters</button>
                  : <button className="btn btn-primary" onClick={() => navigate("/new-entry")}>Create Entry</button>
              }
            />
          )}
