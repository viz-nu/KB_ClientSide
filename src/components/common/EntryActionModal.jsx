...
export default function EntryActionModal({
  ...
  const canSubmit =
    !submitting && (!requiresRemark || remark.trim().length > 0);
  ...
  return (
    ...
    >
      <p style={{ fontSize: 13, color: "var(--text2)", marginBottom: 16 }}>
        Entry: <strong style={{ color: "var(--text)" }}>{entryLabel}</strong>
      </p>

      <FormField
        label={
          requiresRemark ? "Reason (required)" : "Approval remark (optional)"
        }
        description={
          requiresRemark ? "Reason for the action" : "Remark for the action"
        }
