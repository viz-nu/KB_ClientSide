...
export function EmptyState({ icon = '📭', title = 'No data', message, action }) {
  return (
    <div style={{ textAlign: 'center', padding: '48px 24px', color: 'var(--text2)' }}>
      ...
      <div style={{ fontFamily: 'var(--font-head)', fontSize: 16, fontWeight: 700, color: 'var(--text)', marginBottom: 6 }}>{title}</div>
      {message && <div style={{ fontSize: 13, maxWidth: 300, margin: '0 auto' }}>{message}</div>}
...
export function PageHeader({ title, subtitle, actions }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
      <div>
        <h2 style={{ fontFamily: 'var(--font-head)', fontSize: 22, fontWeight: 800 }}>{title}</h2>
...
export function StatCard({ icon, number, label, color = 'var(--accent)', glow }) {
  return (
    <div className="stat-card" style={{ '--stat-color': color, '--stat-glow': glow || `${color}18` }}>
...
export function HealthBar({ label, value, color }) {
  const getColor = (v) => v > 80 ? '#EF4444' : v > 60 ? '#FBBF24' : color || '#22C55E';
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      <div className="health-bar">
        <div className="health-fill" style={{ width: `${value}%`, background: getColor(value) }} />
      </div>
    </div>
  );
}
        ...
        >
          ...
          {t.count != null && (
            <span style={{
              ...
              background: active === t.id ? 'rgba(244,160,28,.2)' : 'rgba(255,255,255,.06)',
...
export function InfoRow({ label, value }) {
  return (
    <div style={{ background: 'rgba(255,255,255,.03)', padding: '10px 12px', borderRadius: 8 }}>
      <div style={{ fontSize: 10, color: 'var(--text2)', marginBottom: 3, textTransform: 'uppercase', letterSpacing: '.07em' }}>{label}</div>
...
export function AlertBanner({ type = 'info', message }) {
  const styles = {
    info:    { bg: 'rgba(59,130,246,.1)', border: 'rgba(59,130,246,.3)', color: '#60A5FA', icon: 'ℹ' },
    warning: { bg: 'rgba(251,191,36,.1)', border: 'rgba(251,191,36,.3)', color: '#FBBF24', icon: '⚠' },
    success: { bg: 'rgba(34,197,94,.1)',  border: 'rgba(34,197,94,.3)',  color: '#22C55E', icon: '✓' },
    error:   { bg: 'rgba(239,68,68,.1)',  border: 'rgba(239,68,68,.3)',  color: '#EF4444', icon: '✕' },
  };
  const s = styles[type];
  return (
    <div style={{ background: s.bg, border: `1px solid ${s.border}`, borderRadius: 8, padding: '10px 14px', display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: 16 }}>
